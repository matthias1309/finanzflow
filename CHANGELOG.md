...
- **Trade Republic PDF import** — `server/pdfParser.ts` now recognizes Trade Republic
  ...
  `parseTradeRepublic` — REQ-005 (AC-005-11). Handles the abbreviated German month date format
  (`28 Aug. 2026`) and derives income/expense from the `TYP` keyword rather than column position;
  rows from the money-market-fund purchase table are intentionally excluded.
