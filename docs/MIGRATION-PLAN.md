...
**Status:** completed (2026-09-24) — two manual items on the Pi remain open (Session 0 credential
rotation, Session 1 first deploy); application bugs found during the migration are follow-ups below
...
### Session 11 — Acceptance & handover ✅ done
Branch: `docs/migration-acceptance`
- [x] `/traceability`, `/test-coverage`, `/system-map` → commit `docs/SYSTEM-MAP.md`
      → traceability: 16/16 REQs → ARCH + TEST-SPEC; code files come from the ARCH lists (⚠️) since
      no implementation commit references a REQ yet. Test coverage: 135 TCs — 87 with a TC comment
      (8 of them 🔴 known-issue regression tests), 47 `accepted`, 1 implicit, **0 missing** after the
      fixes below. System map: 5 domain clusters, hotspots `transactions` and `GET /api/summary/:month`
- [x] First CR for the migration itself → **`CR-005`** (CR-001…004 were already used by the
      Session 5/6/7/9 retrofit reviews), status `approved`
      → fixed in passing: TC-007-01/02/04–08 had been neither closed nor `accepted` in Session 10
      (now `accepted (Session 11)`); E2E specs and four `known issue` tests got their missing TC
...
Changes worth taking back to https://github.com/matthias1309/template (none of them
FinanzFlow-specific):

1. `settings.json`: drop the pinned `model` (outdated `claude-sonnet-4-5`) and the `env` block
   (`NODE_ENV=development` leaks into builds/tests run by Claude); narrow allowlist, extended deny
   list (force-push variants, `git reset --hard`, `git filter-repo`, reading `.env`/`*.db`).
2. Hooks: post-edit hook runs the linter on the edited file and exits 2 on findings (instead of
   only printing a hint); drop the pre-tool-use logging hook.
3. `system-map.md`: remove the "Project_Buddy" / FEAT-lineage leftovers and the undefined
   "V-Model step 0 Impact Analysis"; point `/new-requirement` and `v-model.md` at the map instead.
4. ID scheme: ARCH/TEST numbers mirror the REQ number (REQ-017 → ARCH-017 → TEST-017).
5. `test-coverage.md`: match every TC ID in a multi-ID comment; report `accepted` gaps and
   `known issue` regression tests separately (Session 11).
6. `testing-practices.md`: the `known issue:` regression-test convention for deliberately deferred
   bug fixes.
7. Coding style: one function-length limit (~30 lines) instead of conflicting values.
8. Migration guidance: a retrofit plan that pre-assigns "CR-001" to the final review collides with
   per-session retrofit CRs — say "next free CR number" instead.
...
- **Flaky API tests on macOS** (found in Session 11, CR-005 finding 3a): supertest's per-request
