...
5. For each REQ, run the following git command to find all commits that reference it:
   ...
   Exclude docs/ files and test files — only list implementation source files (`server/`, `client/`, `shared/`).
   Commits from before the migration rarely reference REQs; if a REQ has no commits, fall back to the
   implementing files listed in its ARCH document and mark them ⚠️ (from ARCH, not git).
...
- `server/routes/auth.ts` — changed in 2 commits
- `server/session.ts` — changed in 1 commit

**How to drill deeper:**
```bash
git log --oneline --grep="REQ-001"          # all commits
git log -p --grep="REQ-001" -- server/ client/ shared/   # full diff of implementation changes
```
