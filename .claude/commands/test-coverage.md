...
3. For each TC ID, search `tests/` (incl. `tests/e2e/`) for a comment referencing it (pattern: `// TC-XXX-YY`).
...
|-----------|-------|-------------|-----------|--------|
| TEST-001 | TC-001-01 | Successful login redirects to "/" | `tests/e2e/login.spec.ts` | ✅ |
| TEST-001 | TC-001-02 | Generic error on wrong credentials | `tests/server/api/auth.test.ts` | ✅ |
| TEST-002 | TC-002-01 | Some scenario | — | ❌ missing |

Legend:
- ✅ = TC comment found in at least one test file
- ❌ = no `// TC-XXX-YY` comment found anywhere in `tests/`
...
> Expected in: `tests/server/unit/pdfParser.test.ts`
